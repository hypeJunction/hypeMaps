...
function list_type_map_view($hook, $type, $return, $params)
{
    ...
    if ($list_type == 'mapbox') {
        ...
        return elgg_view('page/components/mapbox', array('list' => $map));
    }
    return $return;
}
/**
...
function setup_site_search_maps($hook, $type, $return, $params)
{
    if (elgg_get_plugin_setting('search_all', PLUGIN_ID)) {
        $return['all'] = array('title' => elgg_echo('maps:search:all'), 'options' => array('id' => 'all', 'types' => 'user'), 'getter' => 'elgg_get_entities', 'access' => 'public', 'priority' => 100);
    }
    if (elgg_get_plugin_setting('search_users', PLUGIN_ID)) {
        $return['users'] = array('title' => elgg_echo('maps:search:users'), 'description' => elgg_echo('maps:search:users:description'), 'options' => array('id' => 'users', 'types' => 'user'), 'getter' => 'elgg_get_entities', 'access' => 'public', 'priority' => 200);
    }
    if (elgg_get_plugin_setting('search_friends', PLUGIN_ID)) {
        $return['friends'] = array('title' => elgg_echo('maps:search:friends'), 'options' => array('id' => 'friends', 'types' => 'user', 'relationship' => 'friend', 'relationship_guid' => elgg_get_logged_in_user_guid()), 'getter' => 'elgg_get_entities', 'access' => 'logged_in', 'priority' => 300);
    }
    if (elgg_get_plugin_setting('search_groups', PLUGIN_ID)) {
        $return['groups'] = array('title' => elgg_echo('maps:search:group'), 'options' => array('id' => 'groups', 'types' => 'group'), 'getter' => 'elgg_get_entities', 'access' => 'public', 'priority' => 400);
    }
    if (elgg_get_plugin_setting('search_objects', PLUGIN_ID)) {
        $return['objects'] = array('title' => elgg_echo('maps:search:objects'), 'options' => array('id' => 'objects', 'types' => 'object', 'subtypes' => get_mappable_object_subtypes()), 'getter' => 'elgg_get_entities', 'access' => 'public', 'priority' => 400);
    }
    return $return;
}
/**
...
function setup_group_search_maps($hook, $type, $return, $params)
{
    ...
    if (elgg_get_plugin_setting('search_group_members', PLUGIN_ID)) {
        $return['group_members'] = array('title' => elgg_echo('maps:search:group_members'), 'options' => array('id' => "group-members-{$group->guid}", 'types' => 'user', 'relationship' => 'member', 'relationship_guid' => $group->guid, 'inverse_relationship' => true), 'getter' => 'elgg_get_entities', 'access' => 'logged_in');
    }
