...
if (!HYPEMAPS_INTERFACE_LOCATION) {
	return true;
}

// elgg_geocode_location() hook
elgg_register_plugin_hook_handler('geocode', 'location', 'hj_maps_geocode_location');

// geocode $entity->location when created and/or updated
elgg_register_event_handler('all', 'metadata', 'hj_maps_geocode_location_metadata');
 ...
 */
function hj_maps_geocode_location($hook, $type, $return, $params) {
 ...
 * @param type $metadata
 */
function hj_maps_geocode_location_metadata($event, $type, $metadata) {
